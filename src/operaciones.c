...
//z80_byte puerto_especial_joystick=0; //Fire Up Down Left Right
    ...
    if ((puerto_h & 8) == 0)   {
        ...
        //gunstick
        if (lightgun_emulation_type==GUNSTICK_SINCLAIR_2) {
            if (mouse_left!=0) {
    ...
    if ((puerto_h & 16) == 0)  {
        ...
        //gunstick
        if (lightgun_emulation_type==GUNSTICK_SINCLAIR_1) {
            if (mouse_left!=0) {
...
z80_byte get_kempston_value(void)
{
                       ...
                       //gunstick
                       if (lightgun_emulation_type==GUNSTICK_KEMPSTON) {
...
//  other bits = Must be "1"    (the programs use compare FDh to test if bit1=0)
               ...
               if (lightgun_emulation_type==GUNSTICK_PORT_DF) {
	...
	*/
	...
	//En Inves, Si A5=0 solamente
        if ( (puerto_l & 32) ==0 && (MACHINE_IS_INVES) ) {
                ...
                if (joystick_emulation==JOYSTICK_KEMPSTON || lightgun_emulation_type==GUNSTICK_KEMPSTON) {
