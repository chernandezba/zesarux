...
int duracion_ultimo_opcode_svi=0;



int core_svi_medio_scanline=0;





void t_scanline_next_fullborder_svi(void)
{
...
void core_svi_fin_frame_pantalla(void)
{
	//Siguiente frame de pantalla
				...
				if (rainbow_enabled.v==1) t_scanline_next_fullborder_svi();
				...
				if (iff1.v==1) {
					...
					if (duracion_ultimo_opcode_svi>=cpu_duracion_pulso_interrupcion) {
...
void core_svi_fin_scanline(void)
{
//printf ("%d\n",t_estados);
			...
			if (1) {
				...
				if (beeper_enabled.v) {
					if (beeper_real_enabled==0) {
						audio_valor_enviar_sonido_izquierdo += da_amplitud_speaker_svi();
						audio_valor_enviar_sonido_derecho += da_amplitud_speaker_svi();
					}
			...
			if (rainbow_enabled.v==1) {
				if (next_frame_skip_render_scanlines) {
					...
					//TIMESENSOR_ENTRY_PRE(TIMESENSOR_ID_core_svi_store_scanline_rainbow);
					//screen_store_scanline_rainbow_solo_border();
					//screen_store_scanline_rainbow_solo_display();

					screen_store_scanline_rainbow_svi_border_and_display();
					//TIMESENSOR_ENTRY_POST(TIMESENSOR_ID_core_svi_store_scanline_rainbow);
				}
			...
			//TIMESENSOR_ENTRY_PRE(TIMESENSOR_ID_core_svi_t_scanline_next_line);
			t_scanline_next_line();
			//TIMESENSOR_ENTRY_POST(TIMESENSOR_ID_core_svi_t_scanline_next_line);
            ...
            if (t_estados>=screen_testados_total) {
				//TIMESENSOR_ENTRY_PRE(TIMESENSOR_ID_core_svi_fin_frame_pantalla);
				core_svi_fin_frame_pantalla();
				//TIMESENSOR_ENTRY_POST(TIMESENSOR_ID_core_svi_fin_frame_pantalla);
			}
			//Fin bloque final de pantalla


    core_svi_medio_scanline=0;

}

void core_svi_handle_interrupts(void)
{
        ...
        z80_adjust_flags_interrupt_block_opcode();
			...
			if (1==1) {
					...
					//e interrupcion nmi tiene prioridad
						if (interrupcion_maskable_generada.v && byte_leido_core_svi!=251) {
						...
						interrupcion_si_despues_lda_ir_svi();
...
void core_svi_ciclo_fetch(void)
{

	//TIMESENSOR_ENTRY_PRE(TIMESENSOR_ID_core_svi_store_rainbow_current_atributes);
	//core_svi_store_rainbow_current_atributes();
	//TIMESENSOR_ENTRY_POST(TIMESENSOR_ID_core_svi_store_rainbow_current_atributes);
	...
	scr_stdout_debug_print_char_routine();
				...
				core_refetch=0;
					...
					byte_leido_core_svi=fetch_opcode();




#ifdef EMULATE_CPU_STATS
				util_stats_increment_counter(stats_codsinpr,byte_leido_core_svi);
#endif

                //Si la cpu está detenida por señal HALT, reemplazar opcode por NOP
                if (z80_halt_signal.v) {
                    byte_leido_core_svi=0;
                }
                else {
                    reg_pc++;
                }
				...
				if (!core_refetch) duracion_ultimo_opcode_svi=t_estados-t_estados_antes_opcode;
				else duracion_ultimo_opcode_svi +=t_estados-t_estados_antes_opcode;
...
//#endif
		...
		if (svi_cas_load_detect() ) {
			...
			if (esperando_tiempo_final_t_estados.v==0) {
				//TIMESENSOR_ENTRY_PRE(TIMESENSOR_ID_core_svi_ciclo_fetch);
				core_svi_ciclo_fetch();
				//TIMESENSOR_ENTRY_POST(TIMESENSOR_ID_core_svi_ciclo_fetch);
            }
    ...
    if (!core_svi_medio_scanline) {
		...
		if ( (t_estados/screen_testados_linea)>t_scanline  ) {
			//TIMESENSOR_ENTRY_PRE(TIMESENSOR_ID_core_svi_fin_scanline);
			core_svi_fin_scanline();
			//TIMESENSOR_ENTRY_POST(TIMESENSOR_ID_core_svi_fin_scanline);
		}


		//Ya hemos leido duracion ultimo opcode. Resetearla a 0 si no hay que hacer refetch
		if (!core_refetch) duracion_ultimo_opcode_svi=0;
		...
		if (interrupcion_maskable_generada.v || interrupcion_non_maskable_generada.v) {
			//TIMESENSOR_ENTRY_PRE(TIMESENSOR_ID_core_svi_handle_interrupts);
			core_svi_handle_interrupts();
			//TIMESENSOR_ENTRY_POST(TIMESENSOR_ID_core_svi_handle_interrupts);
        }
