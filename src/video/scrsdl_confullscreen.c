...
int ancho_sin_escalar=0;
int alto_sin_escalar=0;
...
int scrsdl_crea_ventana(void)
{
    ...
    if (ventana_fullscreen) {
        ...
        ancho_sin_escalar=alto_sin_escalar=0;
        ...
        if (modes != (SDL_Rect **)0 && modes != (SDL_Rect **)-1) {
            ...
            printf("Max monitor: %d X %d\n",monitor_w,monitor_h);

            int ancho_sin_zoom=screen_get_window_size_width_no_zoom_border_en();

            ancho_sin_zoom +=screen_get_ext_desktop_width_no_zoom();

            int alto_sin_zoom=screen_get_window_size_height_no_zoom_border_en();

            alto_sin_zoom +=screen_get_ext_desktop_height_no_zoom();

            //Recalcular zoom
            int posible_zoom_x=monitor_w/ancho_sin_zoom;
            int posible_zoom_y=monitor_h/alto_sin_zoom;

            int zoom_final;

            //nos quedamos con el menor (o igual)
            if (posible_zoom_x>posible_zoom_y) zoom_final=posible_zoom_y;
            else zoom_final=posible_zoom_x;

            zoom_x=zoom_y=zoom_final;

            printf("Zoom: %d\n",zoom_x);

            //recalculo final
            ancho_sin_escalar=screen_get_window_size_width_zoom_border_en();

            ancho_sin_escalar +=screen_get_ext_desktop_width_zoom();

            alto_sin_escalar=screen_get_window_size_height_zoom_border_en();

            alto_sin_escalar +=screen_get_ext_desktop_height_zoom();

            ancho=ancho_sin_escalar;
            alto=alto_sin_escalar;

            ancho_ventana=monitor_w;
            alto_ventana=monitor_h;
    ...
    debug_printf (VERBOSE_DEBUG,"Creating window %d X %d",ancho_ventana,alto_ventana );
...
void scrsdl_putpixel_final_rgb_32_sin_fullscreen(int x,int y,unsigned int color_rgb)
{
...
{
    if (!ventana_fullscreen || !monitor_w || !monitor_h || !ancho_sin_escalar || !alto_sin_escalar) {
        scrsdl_putpixel_final_rgb_32_sin_fullscreen(x,y,color_rgb);
        return;
    }
//scrsdl_putpixel_final_rgb_32_sin_fullscreen(x,y,color_rgb);
//return;
    //zoom decimales
    float zoom_decimal_x=((float)monitor_w)/((float)ancho_sin_escalar);
    float zoom_decimal_y=((float)monitor_h)/((float)alto_sin_escalar);

    //nos quedamos el mas bajo para no deformar
    float zoom_decimal;
    if (zoom_decimal_x<zoom_decimal_y) zoom_decimal=zoom_decimal_x;
    else zoom_decimal=zoom_decimal_y;

//temp
//zoom_decimal=1.2;

    //pos final
    float x_final=((float)x)*zoom_decimal;
    float y_final=((float)y)*zoom_decimal;
//printf("ancho: %d\n",ancho_sin_escalar);
//printf("zoom_decimal: %f\n",zoom_decimal);
    scrsdl_putpixel_final_rgb_32_sin_fullscreen(x_final,y_final,color_rgb);
    //bucle por todos los que hay que escribir

    scrsdl_putpixel_final_rgb_32_sin_fullscreen(2400,y,0xff0000);
    scrsdl_putpixel_final_rgb_32_sin_fullscreen(1200,y,0xff0000);
    scrsdl_putpixel_final_rgb_32_sin_fullscreen(800,y,0xff0000);

    scrsdl_putpixel_final_rgb_32_sin_fullscreen(1055,y,0xff0000);
}
...
void scrsdl_refresca_pantalla_solo_driver(void)
{
    ...
    if (ventana_fullscreen) {
        SDL_UpdateRect(sdl_screen, 0, 0, monitor_w, monitor_h );
    }
    else {
