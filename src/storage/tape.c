        ...
        if (MACHINE_IS_SPECTRUM && noautoload.v && realtape_inserted.v==0) {
            if (reg_pc>=1517 && reg_pc<=1530) {
                ...
                if (en_rom_load) {
                    ...
                    debug_printf(VERBOSE_INFO,"Inserting tape when CPU was on the rom load routine");
...
if (tape_out_file!=0) {
                    ...
                    if (!(MACHINE_IS_ZX80_TYPE)) {
                        ...
                        else if (!util_compare_file_extension(tape_out_file,"p81") ) {
                                        debug_printf (VERBOSE_INFO,"Out .P81 file detected");
                                        if (!(MACHINE_IS_ZX81_TYPE)) {
                                                debug_printf (VERBOSE_ERR,"Out Tape format only supported on ZX81 models");
                                                tape_out_file=NULL;
                                        }
...
int tap_open(void)
{
    ...
    if (tapefile!=0) {
        ...
        //if (noautoload.v==0) {
            ...
            if (fast_autoload.v && !MACHINE_IS_MSX && !MACHINE_IS_SVI && !MACHINE_IS_ACE && !MACHINE_IS_TBBLUE) {
...
loader_detect_loader( void )
{
  ...
  if( storage_accelerate_loading.v && realtape_playing.v )
