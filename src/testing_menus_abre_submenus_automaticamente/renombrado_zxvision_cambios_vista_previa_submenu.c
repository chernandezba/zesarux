...
//Estados:
//0: nada
//1: se entra en submenu
//2: se vuelve de un submenu por tanto hay que atender la tecla pulsada en menu_dibuja_menu_entrando_en_submenu_tecla
int menu_dibuja_menu_entrando_en_submenu=0;
int menu_dibuja_menu_entrando_en_submenu_tecla=0;
...
int menu_dibuja_menu(int *opcion_inicial,menu_item *item_seleccionado,menu_item *m,char *titulo_en,char *titulo_es,char *titulo_ca)
{
    ...
    printf("Entrando en menu_dibuja_menu titulo [%s]\n",titulo);
...
//printf ("despues menu_dibuja_ventana_ret_ancho_titulo\n");
	...
	while (tecla!=13 && tecla!=32 && tecla!=MENU_RETORNO_ESC && tecla!=MENU_RETORNO_F1 && tecla!=MENU_RETORNO_F2 &&
        ...
        int no_mostrar_submenu=0;


        //Si venimos de vista previa de un submenu, esperar tecla y salir simulando flecha izquierda para salir de este submenu
        if (menu_dibuja_menu_entrando_en_submenu==1) {
            printf("Vista previa de un submenu. Esperamos tecla y salimos\n");
            menu_espera_tecla();
            menu_dibuja_menu_entrando_en_submenu=2;
            menu_dibuja_menu_entrando_en_submenu_tecla=zxvision_read_keyboard();
            printf("Salir de vista previa de un submenu\n");
            //return MENU_RETORNO_ESC;
            tecla='5';
            menu_espera_no_tecla();
        }

        //Volvemos de vista previa de un submenu
        else if (menu_dibuja_menu_entrando_en_submenu==2) {
            int previa_tecla=menu_dibuja_menu_entrando_en_submenu_tecla;
            printf("Volvemos de vista previa de un submenu. Atender a tecla pulsada en esa vista previa (%d)\n",previa_tecla);

            //conversion de cursores
			if (previa_tecla==11) tecla='7';
			else if (previa_tecla==10) tecla='6';

            //Ha pulsado cursor izquierda. Por tanto, nos mantenemos en este menu
            else if (previa_tecla==8) {
                tecla=0;
                no_mostrar_submenu=1;
            }

            //Pulsado cursor derecha
            else if (previa_tecla==9) {
                tecla='8';
                no_mostrar_submenu=1;
            }

			else if (previa_tecla==2) tecla=MENU_RETORNO_ESC;

			else if (previa_tecla==3) tecla=MENU_RETORNO_BACKGROUND;

            else tecla=previa_tecla;


            menu_dibuja_menu_entrando_en_submenu=0;
        ...
        printf("tecla al entrar en bucle: %d\n",tecla);
		...
		while (tecla==0 && redibuja_ventana==0 && menu_tooltip_counter<TOOLTIP_SECONDS) {
            ...
            //Si item tiene submenu, automaticamente hacer que entramos
            menu_item *item_test_submenu;
            t_menu_funcion_activo item_test_sel_activo;
            int item_test_submenu_activo=1;

            item_test_submenu=menu_retorna_item(m,(*opcion_inicial));


            item_test_sel_activo=menu_retorna_item(m,(*opcion_inicial))->menu_funcion_activo;
            ...
            if (!no_mostrar_submenu && item_test_submenu->tiene_submenu && item_test_submenu_activo) {
                printf("Tiene submenu\n");
                menu_dibuja_menu_entrando_en_submenu=1;
                //Y saldremos como si el usuario hubiera pulsado enter
                tecla_leida=13;
                //Y movemos el cursor para abajo
...
*/
    ...
    printf("A punto de salir de menu_dibuja_menu\n");
